...
<script src="/App_Plugins/UmbracoFileSystemProviders/ObjectStorage/Install/Configurator/Controllers/Configure.js"></script>

<div ng-app="UFSPLoader">
    <div ng-controller="Loader">
        <div class="row">        
            <div class="span1">
                <img src="/App_Plugins/UmbracoFileSystemProviders/ObjectStorage/Install/azure-logo-32.png"/>
            </div>
            <div><h4>Umbraco ObjectStorage File System Provider</h4></div>
        </div>
        <div class="row">
            <div><hr /></div>
        </div>
        <div class="row" ng-show="!saved">  
            <div>
                <fieldset>
                    <legend><h4>To complete installation, please enter the required parameters for the ObjectStorage provider below</h4></legend>             
                    <form name="paramForm" class="form-horizontal" role="form">             
                        <div ng-repeat="param in parameters" class="control-group">
                            ...
                            <label class="control-label" for="param.key">{{ capitalizeFirstLetter(param.key) }}</label>
                                <div class="controls">
                                    <span ng-if="getInputType(param.key) === 'checkbox'" ng-include="'/App_Plugins/UmbracoFileSystemProviders/ObjectStorage/Install/Configurator/Views/checkbox.htm'"></span>
                                    <span ng-if="getInputType(param.key) === 'text'" ng-include="'/App_Plugins/UmbracoFileSystemProviders/ObjectStorage/Install/Configurator/Views/textfield.htm'"></span>
                                </div>
        ...
        <div class="row">
            <div>
                <div class="alert alert-success" ng-show="saved && (status === 'Ok' || status === 'ImageProcessorWebCompatibility' || status === 'ImageProcessorWebConfigError')">
                   The ObjectStorage storage provider was sucessfully configured and your media is now as light as candyfloss
                </div>
                ...
                <div class="alert alert-error" ng-show="!saved && status === 'ConnectionError'">
                    <strong>Oh no</strong>, there was something wrong with your ObjectStorage connection string, please check and try again, more info in the Umbraco log files
                </div>    
            </div>
        </div>
